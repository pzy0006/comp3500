/* 
 * This is a sample program for runprogram.c
 * Although this file existed prior to project 4, you need completely
 * rewrite the source code for project 4.
 * 
...
loadexec(char *path, vaddr_t *entrypoint, vaddr_t *stackptr)
{
	...
	oldvm = _____________; /* backup t_vmspace of curthread */
	_____________ = newvm; /* setup t_vmspace of curthread to newvm */
	as_activate(_____________); /* activate t_vmspace of curthread */

	/* load the executable. if it fails, restore the old address space and
	 * activate it.
	 */
	result = load_elf(_____________, _____________);
	if (_____________) {
		vfs_close(v);
	...
	result = as_define_stack(_____________, _____________);
	if (_____________) {
		curthread->t_vmspace = oldvm;
		as_activate(curthread->t_vmspace);
		as_destroy(newvm);
	 ...
	 */
	if (oldvm) {
		as_destroy(_____________); /* wipe out old address space oldvm */
	}
	...
	curthread->t_name = _____________; /* set t_name to newname for thread */
...
sys_execv(userptr_t prog, userptr_t argv)
{
	...
	if (argdata.buffer == _____________) {
		lock_release(argdata.lock);
		kfree(path);
		return ENOMEM;
	}
	argdata.offsets = kmalloc(NARG_MAX * sizeof(size_t));
	if (argdata.offsets == _____________) {
		kfree(argdata.buffer);
		lock_release(argdata.lock);
		kfree(path);
		return ENOMEM;
	...
	/* do the copyin */
	result = copyin_args(_____________, _____________);
	if (result) {
		kfree(path);
		kfree(argdata.buffer);
		kfree(argdata.offsets);
	...
	/* load the executable. Note: must not fail after this succeeds. */
	result = loadexec(_____________, _____________, _____________);
	if (result) {
		kfree(path);
		kfree(argdata.buffer);
		kfree(argdata.offsets);
	...
	result = copyout_args(_____________, _____________, _____________);
	if (result) {
		lock_release(_____________);
